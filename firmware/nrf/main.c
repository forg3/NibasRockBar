...
static uint8_t              m_adv_handle = BLE_GAP_ADV_SET_HANDLE_NOT_SET; // handle do advertising set

/* --- 0. SoftDevice: habilita a pilha BLE (S112) --- */
...
/* --- 1. Energia: habilita DC/DC interno no boot (apos o SoftDevice) --- */
...
static void advdata_update(void)
{
    ...
    // S112 7.x: atualiza o payload do advertising set ja configurado (params NULL mantem os atuais)
...
static void advertising_init(void)
{
    ...
    m_adv_params.interval        = ADV_INTERVAL_UNITS; // unidades de 0.625 ms
    m_adv_params.duration        = 0; // sem limite de tempo

    advdata_update();

    // S112 7.x: cria o advertising set com dados + params em uma chamada
...
int main(void)
{
    ble_stack_init();        // habilita S112 antes de qualquer sd_* call
    power_init();            // DC/DC (requer SoftDevice ativo)
    ...
    err_code = app_timer_start(m_battery_timer, APP_TIMER_TICKS(ADV_INTERVAL_MS), NULL);
    APP_ERROR_CHECK(err_code);
