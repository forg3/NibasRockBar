...
| Warnings (all-track-errors) | 54 warning (silk sobre cobre/sobreposição, texto, 2 track_dangling) — não bloqueiam | 36 warning (silk/texto, 1 padstack) — não bloqueiam | **13 warning** de furo: 12× `hole_to_hole` (vias GND de stitch vs PTHs de U1/J1/SW1) + 1× `holes_co_located` — documentados, pendência de fabricação |
